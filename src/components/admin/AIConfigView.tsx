...
export function AIConfigView() {
    ...
    const loadAllSettings = async () => {
        ...
        try {
            const [settings, history, models] = await Promise.all([
                apiGet<AppSettings>('/api/app-settings'),
                apiGet<PromptHistoryItem[]>('/api/prompt-history'),
                apiGet<AiModel[]>('/api/ai-models')
            ]);
    ...
    useEffect(() => {
        async function checkStatus() {
            ...
            try {
                const { isApiKeySet } = await apiGet<{ isApiKeySet: boolean }>('/api/ai-config-status');
    ...
    const handleDeletePrompt = async () => {
        ...
        try {
            await apiDelete<{ success: boolean }>(`/api/prompt-history/${promptToDelete.id}`);
            toast({
                 ...
                 <Card className="premium-surface flex flex-col h-full">
                    ...
                    <CardContent className="space-y-4 flex-grow">
                        ...
                        {loadingStatus ? (
                            ...
                            <Alert variant="default" className="bg-secondary/20 border-secondary/40 text-foreground">
                                <CheckCircle2 className="h-4 w-4 text-secondary" />
                                <AlertTitle className="text-foreground">¡Conectado!</AlertTitle>
