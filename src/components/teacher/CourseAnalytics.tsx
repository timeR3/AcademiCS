...
export function CourseAnalytics({ onBack }: CourseAnalyticsProps) {
    ...
    useEffect(() => {
        if (activeCourse) {
            setLoading(true);
            fetchCourseAnalytics(activeCourse.id)
                .then(data => {
                    setAnalytics(data);
                })
                .catch(err => {
    ...
    const statCards = [
        { title: "Tasa de Finalización", value: `${analytics.completionRate.toFixed(1)}%`, icon: CheckCircle, color: "text-green-500" },
        { title: "Estudiantes Activos", value: analytics.activeStudents, icon: Users, color: "text-blue-500" },
        { title: "Puntuación Promedio", value: `${analytics.averageScore.toFixed(1)}%`, icon: Star, color: "text-yellow-500" },
        { title: "Módulos Completados", value: analytics.totalModulesCompleted, icon: Percent, color: "text-purple-500" }
    ];
