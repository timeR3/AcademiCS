...
import { generateQuestionnaire, saveQuestionnaire } from '@/app/actions';
...
export function QuestionnaireGenerator({ level, isLoading, onQuestionnaireGenerated }: QuestionnaireGeneratorProps) {
  ...
  const handleSave = async () => {
      ...
      try {
        ...
        await refreshCourses(); // Refresh global state to show updated data everywhere
      } catch(e: any) {
        ...
        {generatedQuestions.length > 0 && (
          <div className="mt-4 space-y-6">
            <div className="space-y-2">
                ...
                <ScrollArea className="space-y-4 rounded-md border bg-background p-4 h-[500px]">
                   <Accordion type="single" collapsible value={editingQuestionIndex !== null ? `q-${editingQuestionIndex}` : undefined} onValueChange={(value) => setEditingQuestionIndex(value ? parseInt(value.split('-')[1]) : null)}>
                    {generatedQuestions.map((q, qIndex) => {
                        ...
                        return (
                        <AccordionItem value={`q-${qIndex}`} key={`q-${qIndex}`} className="mb-2 border bg-card rounded-md">
                            ...
                            <AccordionContent className="p-4 pt-0 space-y-4">
                                ...
                                <RadioGroup value={q.correctOptionIndex.toString()} onValueChange={(value) => handleCorrectOptionChange(qIndex, parseInt(value))}>
                                    {q.options.map((opt, oIndex) => (
                                        <div key={`q-${qIndex}-opt-${oIndex}`} className={cn("flex items-start gap-3 rounded-md border p-3 transition-colors", q.correctOptionIndex === oIndex && "bg-primary/5 border-primary")}>
                                            <div className="mt-1">
