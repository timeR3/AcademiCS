...
import type { CourseLevel, CourseSourceFile, SyllabusSection, CourseCategory, CreateSyllabusOutput, StructuredContent } from '@/types';
import { useAuth } from '@/context/AuthContext';
import { createCourse, updateCourseTitle, saveLearningPath, generateSingleModuleSyllabus } from '@/app/actions';
...
export function CourseCreationView({ onCourseSaved, isEditing = false, isAdminView = false }: CourseCreationViewProps) {
  ...
  useEffect(() => {
    const processNextModule = async () => {
        ...
        try {
            const detailedModule = await generateSingleModuleSyllabus({
  ...
  const handleSyllabusIndexGenerated = useCallback((output: CreateSyllabusOutput) => {
      ...
      toast({ title: `Fase 1 y 2 Completadas`, description: `Se ha propuesto un índice de ${output.moduleTitles.length} módulos y se ha clasificado el contenido. Iniciando generación...`});
  ...
  const handleSaveTitle = async () => {
      ...
      try {
          ...
          if ((isEditing || isAdminView) && courseId) {
              await updateCourseTitle({ courseId, title, categoryId: categoryIdNumber });
              toast({ title: '¡Título y Categoría Actualizados!', description: 'La información del curso ha sido guardada.' });
              await refreshCourses();
          } else if (!courseId) {
              const { courseId: newCourseId } = await createCourse({ title, teacherId: user.id, categoryId: categoryIdNumber });
  ...
  const handleSaveLearningPath = async () => {
    ...
    setIsLoading(true);
    try {
        const { updatedLearningPath } = await saveLearningPath({ 
            courseId, 
  ...
  const displayPath = generatedLearningPath.length > 0 ? generatedLearningPath : (levels.map(l => ({ ...l, generationStatus: 'completed' as ModuleGenerationStatus })));
                    ...
                    <AccordionItem value={level.id} key={level.id}>
                        ...
                        <AccordionContent className="p-4 bg-secondary/30 rounded-md space-y-4">
                            ...
                            <>
                                ...
                                <Accordion type="single" collapsible className="w-full bg-background rounded-md p-2 space-y-2">
                                    ...
                                    <AccordionItem value={`section-${sIndex}`} key={sIndex} className="border bg-card rounded-md px-4">
