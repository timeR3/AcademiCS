...
import { enrollStudents, fetchAllUsers } from '@/app/actions';
...
export function StudentManagement() {
    const { activeCourse, updateCourse } = useCourse();
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-4 max-h-[400px] overflow-auto pr-2">
                    {(filteredStudents.length > 0 ? filteredStudents : allPlatformStudents).map(student => {
                        ...
                        return (
                        <div key={student.id} className={cn("flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-md border p-3 gap-3 sm:gap-0", isSelected && "bg-primary/5")}>
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            ...
                                            >
                                                ...
                                                {dueDate && isValid(dueDate) ? format(dueDate, "PPP", { locale: es }) : <span>Fecha límite (opcional)</span>}
                                            </Button>
                                        </PopoverTrigger>
