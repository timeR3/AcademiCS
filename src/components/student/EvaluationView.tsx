...
export function EvaluationView({ level, courseId, onFinish, onBack }: EvaluationViewProps) {
  ...
  const handleAnswerChange = (value: string) => {
    ...
    const optionIndex = questions[currentQuestionIndex].options.findIndex(opt => opt === value);
          ...
          <CardContent>
            ...
            <RadioGroup value={selectedOptionValue} onValueChange={handleAnswerChange} className="space-y-2">
              {currentQuestion.options.map((option, index) => (
                <div key={index} className="flex items-center space-x-3 rounded-md border p-3 hover:bg-secondary has-[[data-state=checked]]:border-primary transition-colors">
