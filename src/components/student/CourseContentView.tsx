...
import { fetchSourceFile, fetchBibliographyItem } from '@/app/actions';
...
export function CourseContentView({ course, onStartEvaluation, onBackToDashboard }: CourseContentViewProps) {
    ...
    const handleDownloadSourceFile = async (file: CourseSourceFile) => {
        ...
        try {
            const { dataUrl, fileName } = await fetchSourceFile(file.id);
            if (dataUrl && fileName) {
    ...
    const handleDownloadBibliographyItem = async (item: CourseBibliographyItem) => {
        ...
        try {
            const { dataUrl, fileName } = await fetchBibliographyItem(item.id);
            if (dataUrl && fileName) {
                downloadFile(fileName, dataUrl);
            } else {
                 throw new Error("El archivo no tiene contenido o el nombre no está disponible.");
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in">
        <div className="mb-6">
            <h1 className="text-4xl font-headline font-bold mb-1">{course.title}</h1>
        ...
        <main className="lg:col-span-3">
             <Tabs defaultValue="content" className="w-full">
                <TabsList>
                    ...
                    <TabsTrigger value="materials">Material de Estudio</TabsTrigger>
                </TabsList>
                    ...
                    {activeSection && activeLevel ? (
                        <ScrollArea className="h-full pr-4">
                            <Card className="shadow-lg bg-card">
                                <CardHeader>
                                    <CardTitle className="font-headline text-3xl">{activeSection.title}</CardTitle>
                                    <CardDescription>
                        ...
                        <div className="flex items-center justify-center h-full text-center py-12 border-2 border-dashed rounded-lg bg-card">
                            <div>
                ...
                <TabsContent value="materials">
                     <Card className="shadow-lg bg-card">
                        <CardHeader>
                            <CardTitle className="font-headline text-3xl">Material de Estudio</CardTitle>
                            <CardDescription>
                                ...
                                {course.sourceFiles && course.sourceFiles.length > 0 ? (
                                    <ul className="space-y-3">
                                        {course.sourceFiles.map(file => (
                                            <li key={file.id} className="flex items-center justify-between p-3 rounded-md border bg-background">
