    ...
    <SidebarProvider>
      <div className="min-h-screen bg-background flex">
        ...
        <div className="flex-1 min-w-0 flex flex-col relative">
          <SeasonalBanner />
          <AppHeader />
          <main className="premium-page flex-1 min-w-0 flex flex-col h-full animate-fade-in">
            <div className="w-full h-full min-w-0 flex flex-col gap-6">
              {sortedRoles.length > 1 && (
                <Tabs value={activeRole} onValueChange={(value) => setActiveRole(value as UserRole)} className="w-full mb-4 sm:mb-6">
                  <TabsList className="grid h-auto w-full grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-1">
                    <TooltipProvider>
                        ...
                        return (
                          <Tooltip key={role}>
                            <TooltipTrigger asChild>
                              <TabsTrigger value={role} className="w-full px-2 py-2 text-xs sm:text-sm">
